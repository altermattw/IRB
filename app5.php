			...
			switch($tempStatus) {
				...
				case "Submitted to IRB":
					if(isAdmin() || $_SESSION["userid"]==current(array_keys($approved["IRB"]))) {
						...
						$message.=$revisionsNeeded;
					} else { // user is not an admin or the IRB reviewer
						if(isAuthor($_SESSION["userid"],$approved)) { // user is an author
							$message.='<p>The application is currently under review by the IRB. However, if you believe that changes need to be made to the application before the IRB approves it, you should press the &ldquo;Revisions Needed&rdquo; button below and explain your concern. That will clear all the approvals and make the application editable again.</p>';
							$message.=$revisionsNeeded;
						}
					}
					break;
